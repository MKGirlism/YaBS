...
while ($stmt->fetch()) {
	...
	if ($del == 0) {
		if ($admin || $owner == $uname) echo "<div id='AddButton' class='AddButton'><a href='editcomment.php?id=$cid'>Edit</a>";
		if ($admin) echo " | <a href='deletecomment.php?id=$cid'>Delete</a>";
		if ($admin || $owner == $uname) echo "</div>";
	}
	else {
		if ($admin) echo "<div id='AddButton' class='AddButton'><a href='deletecomment.php?id=$cid'>Undelete</a></div>";
	}
...
?>
<form name="form1" method="post" action="commentpost.php?id=<? echo $id; ?>">
Add Reply<br />
<br />
