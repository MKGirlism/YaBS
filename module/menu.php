...
if (!empty($_SESSION['uname'])) {
        echo "<a href='profile.php?uid=$i'>".htmlentities($u, ENT_QUOTES, 'UTF-8')."</a>";
}
else {
        echo "<a href='#'>Guest</a>";
}
...
// Show if Logged In.
if (!empty($_SESSION['uname'])) {
	echo "<li><a href='uploader.php'>Uploader</a></li>";
...
if ($admin) echo " | <a href='addpage.php'>Add</a>";
echo "</th></tr>";
echo "<tr class='NavCell0'><td>";
...
while ($stmt->fetch()) {
        if ($priv <= 0 || $admin) echo "<li><a href='page.php?id=$id'>$title</a></li>";
}
echo "</ul></td></tr>";
...
if ($admin) echo " | <a href='addlink.php'>Add</a>";
echo "</th></tr>";
echo "<tr class='NavCell0'><td>";
echo "<ul id='userpanel' class='stackedmenu'>";
...
while ($stmt2->fetch()) {
	...
	if ($admin) echo " <a href='deletelink.php?id=$id2'>X</a>";
	echo "</li>";
}
